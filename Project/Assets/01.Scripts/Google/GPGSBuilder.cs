...
public class GPGSBuilder : MonoBehaviour
{
   static GPGSBuilder inst = new GPGSBuilder();
    public static GPGSBuilder Inst => inst;
        ...
        SavedGame.OpenWithAutomaticConflictResolution(fileName, DataSource.ReadCacheOrNetwork,
            ConflictResolutionStrategy.UseLastKnownGood, (status, game) =>
            {
                if (status == SavedGameRequestStatus.Success)
                {
                    ...
                    SavedGame.CommitUpdate(game, update, bytes, (status, game) =>
                    {
                        onCloudSaved?.Invoke(status == SavedGameRequestStatus.Success);
                    });
                }
            });
    }
    ...
    public void LoadCloud(string fileName, Action<bool, string> onCloudLoaded = null)
    {
        SavedGame.OpenWithAutomaticConflictResolution(fileName, DataSource.ReadCacheOrNetwork,
            ConflictResolutionStrategy.UseLastKnownGood, (status, game) =>
            {
                if (status == SavedGameRequestStatus.Success)
                {
                    SavedGame.ReadBinaryData(game, (status, loadedData) =>
                    {
                        if (status == SavedGameRequestStatus.Success)
                        {
